        ...
        for iteration, (in_part, out_part) in enumerate(dataloader):
            ...
            for iter_frame in range(opt.iter_frames):
                ...
                if iter_frame == 0:
                    ...
                    y_t_warp = pwcnet.PWCNetBackward((p_t_last + 1) / 2, - o_t_last_2_t)
                else:
                    x_t_last = in_part[iter_frame - 1].cuda()
                    p_t_last = p_t.detach()
                    p_t_last.requires_grad = False
                    ...
                    y_t_warp = pwcnet.PWCNetBackward((p_t_last + 1) / 2, o_t_last_2_t)
                    y_t_warp_long = pwcnet.PWCNetBackward((p_t_0 + 1) / 2, o_t_first_2_t)
                # Generator output
                p_t, lstm_state = generatorNet(x_t, p_t_last, lstm_state)
                lstm_state = utils.repackage_hidden(lstm_state)
                if iter_frame == 1:
                    loss_flow += criterion_L1((p_t + 1) / 2, y_t_warp)
                    loss_flow_long += criterion_L1((p_t + 1) / 2, y_t_warp)
                elif iter_frame > 1:
                    loss_flow += criterion_L1((p_t + 1) / 2, y_t_warp)
                    loss_flow_long += criterion_L1((p_t + 1) / 2, y_t_warp_long)
                # Fake samples
